...
export class BitState {
    private _valueEncrypt: number = 0;
     ...
     */
    public constructor(value: number = 0, isEncrypt: boolean = false, public zipBitNum: number = BitUtil.MAX_BIT_FOR_NUM, public cryptTypeArr: Array<BitCryptType> = null, public useBitNum = BitUtil.MAX_BIT_FOR_NUM) {
     ...
     * @param useBitNum 最大BitUtil.MAX_BIT_FOR_NUM，建议不要修改
     */
    public reset(value?: number, isEncrypt?: boolean, zipBitNum?: number, cryptTypeArr?: Array<BitCryptType>): BitState {
    ...
    public set isEncrypt(isEncrypet: boolean) {
        ...
        this._valueEncrypt = this.value;//重新赋值成解密后的
        this._isEncrypt = isEncrypet;
        this.value = this._valueEncrypt;//如果有开启加密则进行加密
    }
    ...
    public set value(value: number) {
        this._valueEncrypt = value;
        if (this._isEncrypt) {
            this._valueEncrypt = this.encrypt()
        }
    }

    public get valueEncrpt(): number {
        return this._valueEncrypt;
    }
    ...
    private cryptCode(isEncrypt: boolean): number {
        var value: number = this._valueEncrypt;//原生数据
        if (!this._isEncrypt || this.cryptTypeArr == null) {//没有字段加密逻辑
            return value;
        }

        var splitNum: number = BitUtil.MAX_BIT_FOR_NUM / this.zipBitNum;
        var bitOne: number = BitUtil.createBitSetValue(this.zipBitNum, 0);
        var splitValue: number;
        for (var i: number = 0; i < splitNum; ++i) {
