...
export class BitUtil {
    ...
    public static setBits(bitState: number, index: number, num: number): number {
        var bitOne: number = BitUtil.createBitSetValue(num, index);
    ...
    public static clearBitsByNum(bitState: number, startIndex: number, clearNum: number): number {
        ...
        //防止倒叙
        var tmp: number = startIndex;
        var endIndex: number = startIndex + clearNum;
    ...
    public static clearBitsByIndexs(bitState: number, startIndex: number, endIndex: number = BitUtil.MAX_BIT_FOR_NUM): number {
        ...
        var bitNum: number = endIndex - startIndex;
        var bitOneValue: number = BitUtil.createBitSetValue(bitNum, startIndex);
    ...
    public static unZipBit(bitState: number, index: number, zipBitNum: number): number {
        var offset: number = index * zipBitNum;
        var bitOneValue: number = BitUtil.createBitSetValue(zipBitNum, offset);
     ...
     * @param endIndex 
     */
    public static bitToArr(bitState: number, startIndex: number = 0, endIndex: number = 64): Array<number> {
        var bitArr: Array<number> = [];
        for (var i: number = startIndex; i < endIndex; ++i) {
     ...
     * @param endIndex 
     */
    public static arrToBit(arr: Array<number>, startIndex: number = 0, endIndex: number = BitUtil.MAX_BIT_FOR_NUM): number {
        var value: number = 0;
        //防御编程
        if (arr == null) {//防空
            return value;
    ...
    public static createBitSetValue(bitSetNum: number, offset: number = 0): number {
        var value: number = 0;
        for (var i: number = 0; i < bitSetNum; ++i) {
